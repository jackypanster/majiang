...
为确保清晰，本文档在每个环节后添加“逻辑验证点”和“潜在边缘case处理”，展开讨论可能的不确定性（如牌墙计算、埋牌影响）。埋牌仅用于定缺验证，不参与牌型或计分；它从起手牌中移除，导致起手牌减少，但最终胡牌时总牌面仍为14张（起手10/11张 + 后续4次摸牌/操作）。
...
### 3.1 回合流程（状态机驱动）
状态机：enum State { Init, Draw, Discard, Respond, HuProcess, EndRound }。从Draw开始，循环至EndRound。

1. **Draw阶段（摸牌）**：
   - if wall_size >0: tile = Wall.pop_back()；hand[current_player].insert(tile)；sort hand。
   - else: set is_huangzhuang=true；goto 流局。
   - if is_supplemental_draw (杠后补)：mark tile as supplemental（用于杠上花）。
   - Check self_hu：call PatternChecker(hand, melds, null)；if true: enqueue Event(Hu, current_player, null, priority=10)。

2. **Discard阶段（打牌）**：
   - 玩家选择tile from hand（AI：评估听牌概率max）。
   - 验证定缺：if hand + melds has missing_suit tiles >0 and tile.suit != missing_suit: invalid（禁止打非缺门）。
   - remove tile from hand；add to public_discard。
   - Enqueue Event(Discard, current_player, tile, priority=1)。

3. **Respond阶段（响应处理）**：
   - Process events queue (pop highest priority first)：
     - **Hu优先 (priority 10)**：for responder in [上家, 对家, 下家] (顺时针)：
       - temp_hand = hand[responder] + tile (as extra)。
       - if suits_in(temp_hand + melds) >2: skip (不缺门)。
       - call PatternChecker(temp_hand, melds, tile) → if can_hu and fan > last_hu_fan[responder] (漏胡): enqueue Hu事件。
       - 一炮多响：允许多个Hu事件，按顺序处理；第一个标记first_hu=true（影响坐庄）。
     - **Kong优先 (8)**：if no Hu: for responder:
       - if tile matches 3 in hand: enqueue Kong(Ming, responder, tile)。
       - if supplemental_draw and meld has Pong of tile: enqueue Kong(Bu, responder, null)。

     - **Pong优先 (5)**：if no Kong/Hu: if tile matches 2 in hand: enqueue Pong。
     - **Pass (0)**：默认过。
   - 执行事件：e.g., Pong: add Meld(Pong, [2 hand + tile])；remove from hand；current_player = responder (抢权)。

   - 最后一张：if wall_size==1: block all Kong events，只Pong/Hu。

4. **血战到底扩展**：
   - After Hu process: hu_count++；set state[hu_player]=Hu。
   - if hu_count <3: continue loop，但skip Hu players (no Draw/Discard)。
   - 已胡玩家：freeze hand/melds；计算final_fan (包括后续杠上)。

5. **EndRound**：current_player = (current +1) %4；if hu_count==3 or huangzhuang: goto结算。
...
### 3.2 特殊事件处理
- **天胡**：Init后，PatternChecker(hand[dealer], melds, null)；if hu: fan+=5；hu_count=1；state=Hu。
- **地胡**：首轮非庄家Draw后立即check；if hu: fan+=5。
- **杠上花**：Kong后Draw：if supplemental and self_hu: fan+=1。
- **杠上炮**：Kong后Discard：响应Hu时 if last_event==Kong: fan+=1。
- **海底捞月**：Draw时 if wall_size==0 pre-draw (last tile): hu时 fan+=1。
- **报叫**：Init后 if Listening (PatternChecker has wait_list.size>0): fan+=2；需玩家声明。
...
2. **Huang Process**：
   - For each player if not Hu:
     ...
     - Buried check: if buried suits not uniform: score -=8*base (or merge to pig: -56)。
