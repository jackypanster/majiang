...
1. **触发条件**：`wall_size == 0` 且 `hu_count == 0`（无人胡牌）。
2. **结算步骤**：
   - 保留所有已写入的杠分结算结果。
   - 不执行查花猪、查大叫或退杠分；直接进入 `EndRound`，按当前分数终止。
3. **庄家处理**：无人胡牌时，默认上一局庄家继续坐庄（即可在 `EndRound` 中保持 `dealer` 不变）。

**逻辑验证点**：仅在“无人胡”时判定流局；若有人胡牌则视为正常结算，即使胡牌发生在最后一轮。
